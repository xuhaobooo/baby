...
class PublishRequire extends Component {
  ...
  publishRequirement = () => {
    ...
    const items = map(serviceItems, (value, key) => ({
  ...
  trustChanged = value => {
    this.setState({ trustCode: value })
  }

  babyChanged = value => {
    this.setState({ babyCode: value })
  ...
  onAddBaby = () => {
    ...
    this.props.dispatch({
      ...
      payload: {
        babyName: addBabyName,
        babySex: addBabySex[0],
        babyBirthday: addBabyBirthday,
      },
  ...
  getDays = () =>
    parseInt(this.state.endTime - this.state.startTime) / 1000 / 60 / 60
  ...
  generateMsg = () => {
    ...
    map(this.state.serviceItems, (value, key) => {
      msg = `${msg + value.itemName}: \t${value.itemPrice}\n`
    })
  ...
  render() {
    ...
    !this.state.babyCode &&
      myBabys.length > 0 &&
      this.babyChanged([myBabys[0].babyCode])
    !this.state.trustCode &&
      trustDict &&
      trustDict.length > 0 &&
      this.setState({ trustCode: [trustDict[0].dicCode] })
    const babyData = map(myBabys, item => ({
      value: item.babyCode,
      label: item.babyName,
    }))
    const trustData = map(trustDict, item => ({
      value: item.dicCode,
      label: item.dicLabel,
    }))
      ...
      <View style={styles.container}>
        ...
        >
          ...
          <InputItem
            ...
            labelNumber={7}
            value={this.state.addBabyName}
            onChange={value =>
              this.setState({ addBabyName: value, addBabyNameError: false })
            }
          >
            姓名:
          </InputItem>
          ...
          <Picker
            data={[{ value: 'M', label: '男' }, { value: 'F', label: '女' }]}
            cols={1}
            title="选择性别"
            onChange={value =>
              this.setState({ addBabySex: value, addBabyNameError: false })
            }
            value={this.state.addBabySex}
          >
            <Item style={styles.selectItem} arraw="horizontal">
              性别
            </Item>
          </Picker>
          <DatePicker
            ...
            title="出生年月"
            onChange={date =>
              this.setState({ addBabyBirthday: date, addBabyNameError: false })
            }
          >
            <Item style={styles.selectItem} arrow="horizontal">
              出生年月
            </Item>
          </DatePicker>

          <WhiteSpace style={{ height: 10, backgroundColor: 'white' }} />
          {this.state.addBabyNameError && (
        ...
        <View
          style={{
            ...
            width: '100%',
            height: ScreenUtil.setSpText(31),
          }}
        >
          <View style={{ flex: 8 }}>
            <Picker
              ...
              cols={1}
              title="选择宝贝"
              onChange={this.babyChanged}
              value={this.state.babyCode}
            >
              <Item style={styles.selectItem} arraw="horizontal">
                宝 贝
              </Item>
          ...
          >
            <Image
              ...
              source={require('../images/user-add.png')}
              resizeMode="stretch"
            />
          </TouchableOpacity>
        </View>
        ...
        <InputItem
          style={styles.itemStyle}
          labelNumber={7}
          value={this.state.babySex === 'M' ? '男' : '女'}
          editable={false}
        >
          性 别
        </InputItem>
        <Picker
          ...
          cols={1}
          title="信任期待"
          onChange={this.trustChanged}
          value={this.state.trustCode}
        >
          <List.Item style={styles.selectItem} arraw="horizontal">
            信任期待
          </List.Item>
        </Picker>
        <View
        ...
        >
          ...
          <TouchableOpacity onPress={this.addrClick}>
            <Image
              style={{
                marginTop: 3,
                width: ScreenUtil.setSpText(20),
        ...
        <InputItem
          ...
          labelNumber={7}
          style={styles.itemStyle}
          value={`${this.state.payMore}`}
          onChange={value =>
            Number.isNaN(Number(value))
              ? Toast.info('请输入数字', 1)
              : this.setState({ payMore: Number(value) })
          }
        >
          附加小费
        </InputItem>
        <View style={{ flex: 10, backgroundColor: 'white' }}>
          <FlatList
            data={serviceWithCatalog}
            extraData={this.state}
            keyExtractor={(item, index) => item.cataCode}
            renderItem={({ item }) => (
              <View>
                <Text
                  style={{
                    ...
                    textAlign: 'center',
                    marginTop: ScreenUtil.setSpText(10),
                  ...
                  {item.list.map(serv => (
                    <Checkbox
                      style={{
                        ...
                        marginLeft: ScreenUtil.setSpText(5),
                        marginBottom: ScreenUtil.setSpText(10),
                      }}
                      key={serv.itemCode}
                      onChange={e => this.serviceChanged(e, serv)}
                      checked={this.state.serviceItems.hasOwnProperty(
                        serv.itemCode
                      )}
                    >
                      <Text
                        style={{
                          width:
                            this.props.app.windowWidth / 3 -
                            ScreenUtil.setSpText(20),
        ...
        <Button
          type="primary"
          style={{ margin: 5, height: ScreenUtil.setSpText(35) }}
          onClick={() =>
            this.validateData() &&
            alert('请确认', this.generateMsg(), [
              { text: '取消' },
