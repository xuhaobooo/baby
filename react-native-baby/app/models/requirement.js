...
export default {
  ...
  effects: {
    ...
    *publishRequire({payload, callback}, { select, call, put }) {
      ...
      yield put(createAction('updateState')({ myRequireList:[] }))
      const date = new Date()
      const startDate = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + ' 00:00:00';
      const endDate = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + ' 23:59:59';
    ...
    *queryPendMyRequire({payload}, { call, put }) {
      const pendRequireList = yield call(requireService.queryPendMyRequire, payload)
      if(pendRequireList){
    ...
    *applyRequire({payload}, { select, call, put }) {
      ...
      const requireList = yield select(state => state.requirement.pendRequireList);
      const newArray = requireList.map((item)=> { 
        if(item.requireCode === requireCode){
          item.applied=true
        }
